...
- **课程详情查看** — 查看每门课程的可用课堂及实时名额，支持全量分页加载
- **自动抢课** — 将目标课程加入抢课列表，系统自动轮询选课，支持冲突确认、登录过期检测。内置**跳过时间冲突**开关（滑动切换），预选结束蹲正选时关闭即可无视冲突持续重试
- **自动替换冲突课程**（每门课独立开关） — 开启后遇时间冲突时自动退掉已选课再选入目标课，适用于替换已中课程
- **可调抢课频率** — 支持自定义选课间隔（同轮内每门课之间）和轮次间隔
- **已选课程管理** — 跨课程池查看已选课程并支持退选
- **端口自动切换** — 启动时若 5000 端口被占用，自动递增查找可用端口
...
启动后访问终端提示的地址（默认 http://127.0.0.1:5000，若端口被占用会自动切换）
...
- **自动抢课页**: 管理抢课列表，每门课记录其所属课程池。启动自动轮询选课，可自定义**选课间隔**（同轮内每门课之间的等待时间）和**轮次间隔**（每轮之间的等待时间）。支持**跳过时间冲突**开关——正选开放前关闭此开关，系统会无视时间冲突持续尝试，开放瞬间即可抢入。每门课支持独立**自动替换**开关——开启后遇冲突自动退旧选新
- **已选课程页**: 跨课程池查看已选上的课程，支持退选
